...
const resolveSubscriptionInvoiceFinancials = (invoice = {}) => {
  ...
  const adjustmentsTotal = Number.isFinite(adjustmentsTotalRaw)
    ? adjustmentsTotalRaw
    : Math.max(0, amount - baseAmount);
  const adjustments = Array.isArray(invoice?.adjustments)
    ? invoice.adjustments
    : [];
  return {
...
const normalizeSubscriptionPaymentApprovalStatus = (
  value = "",
  paymentMethod = "",
) => {
  const normalized = (value || "").toString().trim().toLowerCase();
  ...
  return normalizeSubscriptionPaymentMethod(paymentMethod) ===
    PAYMENT_METHODS.EFT
...
const formatSubscriptionPaymentMethodLabel = (paymentMethod = "") =>
  normalizeSubscriptionPaymentMethod(paymentMethod) === PAYMENT_METHODS.EFT
    ? "EFT"
    : "PayFast";
...
const formatPaymentStatusBadge = (
  invoiceStatus,
  paymentMethod,
  approvalStatus,
) => {
  ...
  const normalizedApproval = normalizeSubscriptionPaymentApprovalStatus(
    approvalStatus,
    normalizedMethod,
  );
  ...
  if (
    ...
    normalizedMethod === PAYMENT_METHODS.EFT
  ) {
    ...
    return {
      label: "Awaiting EFT Approval",
      className: "badge--stock-pending",
    };
  }
  ...
  if (
    normalizedStatus === "pending-payment" &&
    normalizedMethod === PAYMENT_METHODS.PAYFAST
  ) {
...
const getTopUpStats = (invoices = [], subscriptionId) => {
  const subscriptionInvoices = invoices.filter(
    (inv) =>
      (inv?.subscriptionId || "").toString().trim() ===
      (subscriptionId || "").toString().trim(),
  );

  const topUps = subscriptionInvoices.filter(
    (inv) =>
      normalizeSubscriptionInvoiceType(inv?.invoiceType) ===
...
const normalizeMondaySlot = (value = "") => {
  const normalized = (value || "").toString().trim().toLowerCase();
  return SUBSCRIPTION_MONDAY_SLOTS.some((entry) => entry.value === normalized)
    ? normalized
    : "";
};
...
const formatMondaySlotLabel = (slot = "") =>
  SUBSCRIPTION_MONDAY_SLOTS.find(
    (entry) => entry.value === normalizeMondaySlot(slot),
  )?.label || "";
...
const getPreviousBillingMonthKey = (monthKey = "") => {
  ...
  return normalizeBillingMonthKey(
...
const formatSubscriptionPlanLabel = (entry = {}) => {
  ...
  const snapshotPlanName = (entry?.subscriptionPlan?.name || "")
    .toString()
    .trim();
...
const getInvoiceDownloadUrl = (invoice = {}) => {
  const documentDownloadUrl = (invoice?.document?.downloadUrl || "")
    .toString()
    .trim();
...
function AccountPage() {
  usePageMetadata({
    ...
    description:
      "Manage your profile, saved addresses, subscriptions, and order history.",
    noIndex: true,
  });
  ...
  const [subscriptionPreferenceBusyId, setSubscriptionPreferenceBusyId] =
    useState("");
  const [subscriptionMondaySlotDrafts, setSubscriptionMondaySlotDrafts] =
    useState({});
  ...
  const generateSubscriptionInvoiceDocumentNow = useMemo(
    () =>
      functionsInstance
        ? httpsCallable(
            functionsInstance,
            "generateSubscriptionInvoiceDocumentNow",
          )
        : null,
    [functionsInstance],
  );
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const rows = snapshot.docs
          ...
          .sort(
            (left, right) =>
              subscriptionSortValue(right) - subscriptionSortValue(left),
          );
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const rows = snapshot.docs
          ...
          .sort(
            (left, right) => invoiceSortValue(right) - invoiceSortValue(left),
          );
    ...
    setSubscriptionMondaySlotDrafts((prev) => {
      ...
      (Array.isArray(subscriptions) ? subscriptions : []).forEach(
        (subscription) => {
          ...
          if (!tier) return;
          const existing = Array.isArray(prev[subscriptionId])
            ? prev[subscriptionId]
            : null;
          const sourceSlots = Array.isArray(
  ...
  useEffect(() => {
    ...
    setAuthMessage(
      (prev) => prev || "Sign in or create your account to continue.",
    );
  ...
  const handleSignIn = async (event) => {
    ...
    try {
      ...
      const resolvedRole = (credentials?.resolvedRole || "")
        .toString()
        .trim()
        .toLowerCase();
  ...
  const handleForgotPassword = async () => {
    if (authBusy) return;
    const email = (
      authMode === "signup" ? signUpForm.email : signInForm.email
    ).trim();
    if (!email) {
    ...
    try {
      ...
      setAuthError(
        formatAuthErrorMessage(error, "Unable to send password reset email."),
      );
    } finally {
      setAuthBusy(false);
    }
  ...
  const handleSubscriptionAction = async (subscriptionId, action) => {
    ...
    try {
      ...
      setSubscriptionStatus(
        `Subscription ${normalizedId.slice(0, 8)} status: ${statusLabel}.`,
      );
    } catch (error) {
      setSubscriptionError(
        error?.message || "Unable to update subscription status.",
      );
    } finally {
  ...
  const sendInvoicePayLink = async (
    subscriptionId,
    { redirectToPay = false } = {},
  ) => {
    const normalizedId = (subscriptionId || "").toString().trim();
    ...
    try {
      ...
      if (data.emailStatus === "sent") {
        setSubscriptionStatus(
          `${sentLabel} for invoice ${invoiceLabel || "update"}.`,
        );
      } else {
      ...
      setSubscriptionError(
        error?.message || "Unable to send a subscription pay link.",
      );
    } finally {
  ...
  const uploadSubscriptionEftProof = async (invoice) => {
    ...
    if (!storage || !attachSubscriptionEftPaymentProof) {
      setSubscriptionError(
        "Subscription proof upload is unavailable right now.",
      );
      return;
    }
    const invoiceStatus = normalizeSubscriptionInvoiceStatus(
      invoice?.status || "",
    );
    ...
    if (
      ...
      setSubscriptionError(
        "Proof can only be uploaded for pending EFT invoices.",
      );
      return;
    }
    ...
    try {
      ...
      setSubscriptionStatus(
        "EFT proof uploaded. Admin will review payment approval.",
      );
    } catch (error) {
      setSubscriptionError(
        error?.message || "Unable to upload subscription payment proof.",
      );
    } finally {
  ...
  const handleToggleSubscriptionMondaySlot = (subscription, slotValue) => {
    ...
    setSubscriptionMondaySlotDrafts((prev) => {
      const current = normalizeMondaySlotsForTier(
        tier,
        prev[subscriptionId] || [],
      );
  ...
  const saveSubscriptionMondaySlots = async (subscription) => {
    ...
    try {
      ...
      const data = result?.data || {};
      const effectiveMonth = normalizeBillingMonthKey(
        data.effectiveFromCycleMonth || "",
      );
      const effectiveLabel = effectiveMonth
        ? formatBillingMonthLabel(effectiveMonth)
        : "the next cycle";
      const slotsLabel = formatMondaySlotList(
        data.mondaySlots || selectedSlots,
      );
      setSubscriptionStatus(
      ...
      setSubscriptionMondaySlotDrafts((prev) => ({
        ...prev,
        [subscriptionId]: normalizeMondaySlotsForTier(
          tier,
          data.mondaySlots || selectedSlots,
        ),
      }));
      ...
      setSubscriptionError(
        error?.message || "Unable to update delivery preferences.",
      );
    } finally {
  ...
  const previewSubscriptionInvoice = async (invoiceId) => {
    ...
    try {
      ...
      const data = result?.data || {};
      const invoiceDownloadUrl = (data.invoiceDownloadUrl || "")
        .toString()
        .trim();
      ...
      setSubscriptionError(
        error?.message || "Unable to preview this invoice right now.",
      );
    } finally {
  ...
  const commitAddressDraft = ({ mode = "add", addressId = "" } = {}) => {
    ...
    if (mode === "edit") {
      ...
      const addressExists = profileForm.addresses.some(
  ...
  const handleRemoveAddress = (addressId) => {
    ...
    setProfileForm((prev) => {
      ...
      const nextDefault =
        prev.defaultAddressId === addressId
          ? nextAddresses[0]?.id || ""
  ...
  const showAuth =
    !user || authBusy || (Boolean(user) && authLoading && role === "guest");
  ...
  const {
    latestPendingInvoiceBySubscription,
    pendingInvoiceCountBySubscription,
  } = useMemo(() => {
    ...
    subscriptionInvoices.forEach((invoice) => {
      ...
      pendingCount.set(
        subscriptionId,
        (pendingCount.get(subscriptionId) || 0) + 1,
      );
  ...
  const currentSubscriptions = useMemo(
    () =>
      subscriptions.filter(
        (subscription) =>
          normalizeSubscriptionStatus(subscription.status) !== "cancelled",
      ),
    [subscriptions],
  );
    ...
    () =>
      new Set(
        currentSubscriptions
          .map((subscription) => (subscription.id || "").toString().trim())
          .filter(Boolean),
      ),
    [currentSubscriptions],
  );
  ...
  const cancelledSubscriptionInvoices = useMemo(
    () =>
      subscriptionInvoices.filter((invoice) => {
        const subscriptionId = (invoice.subscriptionId || "").toString().trim();
        if (subscriptionId && cancelledSubscriptionIds.has(subscriptionId))
          return true;
        const invoiceStatus = normalizeSubscriptionInvoiceStatus(
          invoice.status,
        );
        ...
        <div className="section__inner admin-auth__inner">
          <Reveal as="div" className="admin-auth__card">
            <div className="admin-auth__header">
              ...
              <p>
                Secure access to your orders, saved addresses, and checkout
                details.
              </p>
            </div>
            ...
            {authMode === "signin" ? (
              <form className="admin-auth__form" onSubmit={handleSignIn}>
                ...
                <label
                  className="admin-auth__field"
                  htmlFor="account-signin-password"
                >
                  <span>Password</span>
                ...
                {authMessage && (
                  <p className="admin-panel__status">{authMessage}</p>
                )}
  ...
  return (
    <section className="section section--tight account-page">
      <div className="section__inner account-page__inner">
        <div className="account-page__header">
          ...
          <p className="modal__meta">
            Save your delivery details once, track your orders, and manage
            subscription preferences.
          </p>
        </div>
        <div className="account-grid">
          <form
            className="admin-panel account-panel"
          ...
          >
            ...
            <div className="account-panel__content">
              ...
              {profileError && (
                <p className="admin-panel__error">{profileError}</p>
              )}
              {profileStatus && (
                <p className="admin-panel__status">{profileStatus}</p>
              )}
            </div>
          </form>
          ...
          <section className="admin-panel account-panel">
            ...
            <div className="account-panel__content">
              ...
              {profileForm.addresses.length === 0 ? (
                ...
                <div className="account-addresses">
                  {profileForm.addresses.map((address) => {
                    const isDefault =
                      profileForm.defaultAddressId === address.id;
                    return (
                      <article
              ...
              <p className="modal__meta">
                Save profile after editing addresses so checkout can auto-fill
                delivery details.
              </p>
            </div>
          </section>
          ...
          <section className="admin-panel account-panel account-panel--subscriptions">
            ...
            <div className="account-panel__content account-subscription-panel">
              <p className="modal__meta">
                Manage your current active and paused subscriptions here. Start
                new subscriptions from the dedicated checkout page, and use
                history for cancelled records.
              </p>
              ...
              {subscriptionsError && (
                <p className="admin-panel__error">{subscriptionsError}</p>
              )}
              {!subscriptionsLoading &&
                !subscriptionsError &&
                currentSubscriptions.length === 0 && (
                  <p className="modal__meta">
                    ...
                    <Link to="/subscriptions/checkout">
                ...
                currentSubscriptions.length > 0 && (
                  <div className="account-subscription-list">
                    {currentSubscriptions.map((subscription) => {
                      ...
                      const statusLabel =
                        SUBSCRIPTION_STATUS_LABELS[normalizedStatus] ||
                        normalizedStatus ||
                        "Active";
                      const latestInvoice =
                        latestInvoiceBySubscription.get(subscription.id) ||
                        null;
                      const pendingInvoice =
                        latestPendingInvoiceBySubscription.get(
                          subscription.id,
                        ) || null;
                      const latestInvoiceFinancials = latestInvoice
                        ? resolveSubscriptionInvoiceFinancials(latestInvoice)
                        : null;
                      const pendingInvoicePaymentMethod =
                      ...
                      const paymentApprovalStatus =
                        normalizeSubscriptionPaymentApprovalStatus(
                      ...
                      const paymentApprovalLabel =
                        formatSubscriptionPaymentApprovalLabel(
                          paymentApprovalStatus,
                        );
                      const isPendingEftInvoice =
                        Boolean(pendingInvoice) &&
                        pendingInvoicePaymentMethod === PAYMENT_METHODS.EFT;
                      const pendingInvoiceCount =
                        pendingInvoiceCountBySubscription.get(
                          subscription.id,
                        ) || 0;
                      const pendingInvoiceTypeLabel =
                        formatSubscriptionInvoiceTypeLabel(
                          pendingInvoice?.invoiceType || "",
                        );
                      const isActionBusy =
                        subscriptionActionBusyId === subscription.id;
                      const isInvoiceBusy =
                        invoiceActionBusyId === subscription.id;
                      const isPreferenceBusy =
                        subscriptionPreferenceBusyId === subscription.id;
                      ...
                      const draftMondaySlots = normalizeMondaySlotsForTier(
                        subscriptionTier,
                        subscriptionMondaySlotDrafts[subscription.id] ||
                          savedMondaySlots,
                      );
                      const mondaySlotsChanged =
                        JSON.stringify(draftMondaySlots) !==
                        JSON.stringify(savedMondaySlots);
                      const isPendingInvoice = Boolean(pendingInvoice);
                      const nextBillingMonth = normalizeBillingMonthKey(
                        subscription?.nextBillingMonth || "",
                      );
                      const payLinkBlockedUntilBillingMonth =
                        !isPendingInvoice &&
                        Boolean(nextBillingMonth) &&
                        compareBillingMonthKeys(
                          currentBillingMonthKey,
                          nextBillingMonth,
                        ) < 0;
                      const nextInvoiceWindowMonth =
                        payLinkBlockedUntilBillingMonth
                          ? getPreviousBillingMonthKey(nextBillingMonth) ||
                            currentBillingMonthKey
                          : "";
                      ...
                      const nextBillingCycleLabel =
                        formatBillingMonthLabel(nextBillingMonth);
                      ...
                      const pendingInvoiceAmount = Number(
                        pendingInvoice?.amount || 0,
                      );
                      const amountDueNow =
                        isPendingInvoice &&
                        Number.isFinite(pendingInvoiceAmount)
                      ...
                      const amountDueExtraMeta =
                        isPendingInvoice && pendingInvoiceCount > 1
                      ...
                      const paidDate = latestInvoiceSettled
                        ? formatPaidDate(latestInvoice)
                        : null;
                      const payFastTransactionId = latestInvoice
                        ? getPayFastTransactionId(latestInvoice)
                        : "";
                      const topUpStats = getTopUpStats(
                        subscriptionInvoices,
                        subscription.id,
                      );
                      const lastPaymentDate =
                        getLastPaymentDate(subscriptionInvoices);
                      ...
                      return (
                        ...
                        >
                          <div className="account-subscription-card__head">
                            ...
                            <span
                              className={`badge badge--stock-${normalizedStatus === "active" ? "in" : "out"}`}
                            >
                              {statusLabel}
                            </span>
                          </div>
                          <div className="account-subscription-card__amount">
                            <span className="account-subscription-card__amount-label">
                              Amount due now
                            </span>
                            <strong>{amountDueNowLabel}</strong>
                            <span
                              className={`badge ${paymentStatusBadge.className}`}
                            >
                            ...
                            {topUpStats.count > 0 && (
                              <div className="account-subscription-fact">
                                ...
                                {topUpStats.pendingCount > 0 && (
                                  <p className="modal__meta">
                                    {topUpStats.pendingCount} pending (
                                    {formatCurrency(topUpStats.pendingAmount)})
                                  </p>
                                )}
                              </div>
                            )}
                          ...
                          <div className="account-subscription-card__facts">
                            ...
                            {subscription?.address && (
                              <div className="account-subscription-fact account-subscription-fact--wide">
                                ...
                                <strong>
                                  {formatShippingAddress(subscription.address)}
                                </strong>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="account-subscription-delivery-preference">
                            ...
                            <p className="modal__meta">
                              {formatMondaySlotList(draftMondaySlots) ||
                                "Not set"}
                            </p>
                            {canEditDeliveryPreferences &&
                              subscriptionTier !== "weekly" && (
                                <>
                                  <p className="modal__meta">
                                    Select{" "}
                                    {resolveRequiredMondaySlotCount(
                                      subscriptionTier,
                                    )}{" "}
                                    Monday
                                    {resolveRequiredMondaySlotCount(
                                  ...
                                  <div className="account-subscription-slot-options">
                                    {SUBSCRIPTION_MONDAY_SLOTS.map((slot) => {
                                      const checked = draftMondaySlots.includes(
                                        slot.value,
                                      );
                                      return (
                                        <label
                                        ...
                                        >
                                          <input
                                            ...
                                            onChange={() =>
                                              handleToggleSubscriptionMondaySlot(
                                                subscription,
                                                slot.value,
                                              )
                                            }
                                  ...
                                  <button
                                    ...
                                    type="button"
                                    disabled={
                                      isPreferenceBusy || !mondaySlotsChanged
                                    }
                                    onClick={() =>
                                      saveSubscriptionMondaySlots(subscription)
                                    }
                                  >
                                    {isPreferenceBusy
                                      ? "Saving..."
                                      : "Save delivery Mondays"}
                                  </button>
                                </>
                              )}
                            {subscriptionTier === "weekly" && (
                              <p className="modal__meta">
                                Weekly plans always deliver on all Mondays.
                              </p>
                            )}
                          </div>
                          {latestInvoice && (
                            <div className="account-subscription-card__invoice">
                              ...
                              <strong>
                                {latestInvoice.cycleMonth || "current"} -{" "}
                                {formatCurrency(
                                  latestInvoiceFinancials?.amount ||
                                    latestInvoice.amount,
                                )}
                              </strong>
                              <p className="modal__meta">
                                {latestInvoiceTypeLabel} | Base{" "}
                                {formatCurrency(
                                  latestInvoiceFinancials?.baseAmount || 0,
                                )}{" "}
                                | Adjustments{" "}
                                {formatCurrency(
                                  latestInvoiceFinancials?.adjustmentsTotal ||
                                    0,
                                )}
                              </p>
                          ...
                          {isPendingEftInvoice && pendingInvoice && (
                            <div className="account-subscription-proof">
                              <p className="modal__meta">
                                EFT payment is pending admin approval. Upload
                                proof of payment (optional) to speed up review.
                              </p>
                              <div className="account-subscription-proof__actions">
                                ...
                                <button
                                  ...
                                  type="button"
                                  disabled={
                                    subscriptionProofBusyId ===
                                      pendingInvoice.id ||
                                    !subscriptionProofFiles[pendingInvoice.id]
                                  }
                                  onClick={() =>
                                    uploadSubscriptionEftProof(pendingInvoice)
                                  }
                                >
                          ...
                          <div className="account-subscription-card__actions">
                            ...
                            <button
                              ...
                              onClick={() =>
                                sendInvoicePayLink(subscription.id, {
                                  redirectToPay: isPendingInvoice,
                                })
                              }
                            >
                              {payActionLabel}
                            ...
                            <button
                              ...
                              type="button"
                              disabled={
                                isInvoiceBusy || payLinkBlockedUntilBillingMonth
                              }
                              onClick={() =>
                                sendInvoicePayLink(subscription.id)
                              }
              ...
              <div className="account-subscription-invoices">
                ...
                {invoicesLoading && (
                  <p className="modal__meta">Loading invoices...</p>
                )}
                {invoicesError && (
                  <p className="admin-panel__error">{invoicesError}</p>
                )}
                {!invoicesLoading &&
                  !invoicesError &&
                  currentSubscriptionInvoices.length === 0 && (
                    <p className="modal__meta">
                      No invoices for current subscriptions yet.
                    </p>
                  )}
                {!invoicesLoading &&
                  !invoicesError &&
                    ...
                    <div className="account-order-items">
                      {currentSubscriptionInvoices.map((invoice) => {
                        ...
                        const invoiceFinancials =
                          resolveSubscriptionInvoiceFinancials(invoice);
                        ...
                        const invoicePaymentApprovalLabel =
                          formatSubscriptionPaymentApprovalLabel(
                        ...
                        const isPreviewBusy =
                          invoicePreviewBusyId === invoice.id;
                        const isProofBusy =
                          subscriptionProofBusyId === invoice.id;
                        ...
                        return (
                          <article
                            key={invoice.id}
                            className="account-order-item-card"
                          >
                            <div className="account-order-item-card__head">
                              <p className="account-order-item-card__name">
                                {invoice.cycleMonth || "Cycle"} -{" "}
                                {formatCurrency(invoiceFinancials.amount)}
                              </p>
                              <span
                                className={`badge badge--stock-${invoiceStatus === "paid" ? "in" : "out"}`}
                              >
                                {SUBSCRIPTION_INVOICE_STATUS_LABELS[
                                  invoiceStatus
                                ] || "Pending payment"}
                            ...
                            <p className="modal__meta">
                              {invoiceTypeLabel} invoice | Base{" "}
                              {formatCurrency(invoiceFinancials.baseAmount)} |
                              Adjustments{" "}
                              {formatCurrency(
                                invoiceFinancials.adjustmentsTotal,
                              )}
                            </p>
                            {invoiceFinancials.adjustments.length > 0 && (
                              <p className="modal__meta">
                                {invoiceFinancials.adjustments
                                  .map(
                                    (entry) =>
                                      `${entry?.label || "Adjustment"}: ${formatCurrency(entry?.amount || 0)}`,
                                  )
                                  .join(" | ")}
                              </p>
                            )}
                            ...
                            <div className="account-subscription-card__actions">
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  previewSubscriptionInvoice(invoice.id)
                                }
                              ...
                              {canUploadProof && (
                                <button
                                  ...
                                  onClick={() =>
                                    uploadSubscriptionEftProof(invoice)
                                  }
                                >
                                  {isProofBusy
                                    ? "Uploading..."
              ...
              {subscriptionError && (
                <p className="admin-panel__error">{subscriptionError}</p>
              )}
              {subscriptionStatus && (
                <p className="admin-panel__status">{subscriptionStatus}</p>
              )}
            </div>
          </section>
          ...
          <section className="admin-panel account-panel account-panel--orders">
            ...
            <div className="account-panel__content">
              {ordersLoading && (
                <p className="modal__meta">Loading your orders...</p>
              )}
              {ordersError && (
                <p className="admin-panel__error">{ordersError}</p>
              )}
              ...
              {!ordersLoading && !ordersError && orders.length > 0 && (
                <div className="account-orders">
                  {orders.map((order) => (
                    <Link
                      className="account-order-card account-order-card--link"
                      key={order.id}
                    ...
                    >
                      <div className="account-order-card__head">
                        ...
                        <span
                          className={`badge badge--stock-${order.paymentStatus === "paid" ? "in" : "out"}`}
                        >
                      ...
                      <p className="modal__meta">
                        Status:{" "}
                        {formatOrderStatusLabel(order.status || "pending")}
                      </p>
                      <p className="modal__meta">
                        Total: R{Number(order.totalPrice || 0).toFixed(2)}
                      </p>
                      <p className="account-order-card__cta">
      ...
      {addressDialogOpen && (
        ...
        >
          <div
            className="modal__content account-address-dialog"
          ...
          >
            ...
            <form
              className="account-address-dialog__form"
        ...
        >
          ...
          >
            ...
            <h3
              ...
              id="account-subscription-history-title"
            >
              Cancelled subscription history
            </h3>
            <p className="modal__meta">
              Archived records for cancelled subscriptions and their related
              invoices.
            </p>
            ...
            <div className="account-subscription-history">
              <section className="account-subscription-history__section">
                ...
                {cancelledSubscriptions.length === 0 ? (
                  <p className="modal__meta">
                    No cancelled subscriptions in history.
                  </p>
                ) : (
                  <div className="account-subscription-list">
              ...
              <section className="account-subscription-history__section">
                ...
                {cancelledSubscriptionInvoices.length === 0 ? (
                  <p className="modal__meta">
                    No cancelled invoices in history.
                  </p>
                ) : (
                  <div className="account-order-items">
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="modal__meta">
                            ...
                            {formatCurrency(invoiceFinancials.baseAmount)} |
                            Adjustments{" "}
                            {formatCurrency(invoiceFinancials.adjustmentsTotal)}
                          </p>
                          <p className="modal__meta">
                            Updated:{" "}
                            {formatOrderDate(
